<?php 
	...
	if(isset($_POST['kullanicikaydet'])){
		...
		$kullanici_passwodone =trim($_POST['kullanici_passwordone']);
		$kullanici_passwodtwo = trim($_POST['kullanici_passwordtwo']);
		if($kullanici_passwodone == $kullanici_passwodtwo){
			if(strlen($kullanici_passwordone) >= 6){
				...
				if ($say==0) {
					$password=md5($kullanici_passwordone); //md5 fonksiyonu şifreyi md5 şifreli hale getirir.
					$kullanici_yetki=1;
				...
				Header("Location:../../register.php?eksiksifre");
			}
		}else{
			Header("Location:../../register.php?farklisifre");
		}
	}
	...
	if (isset($_POST['iletisimayarkaydet'])) {
		...
		if ($update) {
			header("Location:../production/iletişim-ayarlar.php?durum=ok");
		}
		else{
			header("Location:../production/iletişim-ayarlar.php?durum=no");
		}
	}
...
if ($_GET['menusil'] == "ok") {
	...
	$kontrol = $sil->execute(array('id' => $_GET['menu_id']));
